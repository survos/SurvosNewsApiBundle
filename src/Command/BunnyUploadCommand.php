...
#[AsCommand('news-api:upload', 'upload remote news-api files')]
final class NewsApiUploadCommand extends InvokableServiceCommand
{
    use RunsCommands;
    use RunsProcesses;
...
bin/console news-api:upload local/path/filename.zip [remote/path/filename.zip] --zone=zoneName
# change name and path
bin/console news-api:upload local/path/filename.zip remote/path/newFilename.zip
# change remote path only, needs slash at the end!
bin/console news-api:upload local/path/filename.zip remote/path/
...
//        #[Option(description: 'dir')] ?string $relativeDir = './',
        ...
        $ret = $this->news-apiService->uploadFile(
            $remoteFilename,
